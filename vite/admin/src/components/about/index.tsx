...
const UsageHelp = () => (
  <>
    <SettingsSection title={t("保存与离开保护")}>
      <Paragraph>
        {t("修改普通设置或填写凭据后，页面会显示待保存数量。刷新、关闭标签页或切换到插件内其他区域时，系统会先确认，避免静默丢失草稿。")}
      </Paragraph>
      <Paragraph>
        {t("保存请求带有读取设置时的配置版本。如果其他管理员、另一个标签页或受控代码已经更新设置，本次保存会停止并提示重新读取，不会用旧快照覆盖新配置。")}
      </Paragraph>
    </SettingsSection>
    <SettingsSection title={t("发生保存冲突时")}>
      <Paragraph>
        {t("先记下当前准备修改的项目，再重新读取或刷新后台。核对最新配置和保存差异后，再提交需要保留的修改。")}
      </Paragraph>
    </SettingsSection>
    <SettingsSection title={t("数据库清理")}>
      <Paragraph>
        {t("数据库清理必须按项目先预览、再确认。预览资格仅对当前管理员和当前清理项目有效，5 分钟后失效且只能使用一次；如果执行前数据库内容发生变化，系统会停止清理并要求重新预览。")}
      </Paragraph>
    </SettingsSection>
    <SettingsSection title={t("图片 Alt 检查")}>
      <Paragraph>
        {t("SEO 检查助手和媒体库体检只检查缺少 Alt 的图片并显示数量。为避免与站点已有 SEO 或媒体插件重复写入，Npcink Site Toolbox 不提供 Alt 自动写入；请在负责 SEO 的插件或媒体库中人工处理。")}
      </Paragraph>
    </SettingsSection>
  </>
);
